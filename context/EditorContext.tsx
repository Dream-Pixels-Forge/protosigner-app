...
export const EditorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getCanvasDims = () => {
    ...
    // Updated width calculation: Layers (288px) + Properties (384px) + Padding (~16px) = 688px
    return { w: window.innerWidth - 688, h: window.innerHeight - 136 };
  };
  ...
  // --- AI Hook Integration ---
  const { isGenerating, generateContent, generateStyles, refineSelectionLayout } = useAI({
      elements,
      setElements,
      selectedId,
  ...
  return (
    <EditorContext.Provider value={{
      ...
      isGenerating, generateContent, generateStyles, refineSelectionLayout, mcpStatus,
