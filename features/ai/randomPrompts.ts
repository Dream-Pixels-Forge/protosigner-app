...
export const RANDOM_PROMPTS: Record<ExpertMode, string[]> = {
    ...
    'grid-master': [
        "A strict 12-column grid layout for a financial report. Sidebar taking 3 columns, main content 9 columns. Inside main content, widgets spanning 3, 6, and 12 columns with precise 20px gaps.",
        "A Swiss Style poster layout. Asymmetrical grid usage, bold Helvetica typography aligned to the grid lines, large negative space, and a single accent color.",
        "A modular magazine cover page. Golden ratio proportions, image containers aligning perfectly with text blocks, 'box-sizing: border-box' evident in the structure.",
        "A complex data table with filters. Header row spanning full width, filter sidebar on left (fixed width), and responsive data columns that collapse gracefully."
    ]
};
